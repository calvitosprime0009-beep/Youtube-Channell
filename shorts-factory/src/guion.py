...
def generar_texto(prompt, sistema=SISTEMA):
    ...
    if groq_key:
        ...
        print("[guion] Advertencia: GROQ_API_KEY no está definida en las variables de entorno.")

    # 2) Pollinations (Respaldo — gratis y sin clave)
    try:
        print("[guion] Intentando respaldo con Pollinations...")
        d = _post(POLLI, {"model": "openai", "messages": msgs, "seed": random.randint(1, 10**6)})
        print("[guion] ¡Pollinations respondió con éxito!")
        return d["choices"][0]["message"]["content"].strip()
    except Exception as e:
